...
import {
  ...
  m, // Main component for animations
...
const LargeDock = ({
  ...
  return (
    <m.div
      ...
      className={cn(
        "mx-auto hidden h-16 items-end gap-4 rounded-2xl bg-white/10 px-4 pb-3  md:flex", // Large dock styles
        className,
  ...
  return (
    <Link href={href}>
      <m.div
        ...
        className="relative flex aspect-square items-center justify-center rounded-full bg-white/20 text-primary-accent shadow-lg backdrop-blur-md dark:bg-black/20 dark:text-white"
      >
        <AnimatePresence>
            ...
            {items.map((item, index) => (
              ...
              >
                <Link
                  href={item.href}
                  key={item.title}
                  className="flex size-10 relative items-center justify-center rounded-full bg-white/20 text-primary-accent shadow-md backdrop-blur-md"
                >
                  <div className="h-4 w-4 absolute top-1/2 left-1/2 -translate-x-[70%] -translate-y-[60%] ">{item.icon}</div> {/* Render the icon */}
                </Link>
              </m.div>
            ))}
          </m.div>
