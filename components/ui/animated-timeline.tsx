    ...
    >
      ...
      <div className={cn("flex-grow leading-5", !isLast && "mb-3")}>
        {customRender ? (
          ...
          <>
            <h3 className="text-lg font-semibold !text-primary-accent" style={{ color: styles.titleColor }}>
              {event.title}
            </h3>
            <p className="italic" style={{ color: styles.descriptionColor }}>{event.description}</p>
            <span className="text-sm font-semibold !text-primary-accent" style={{ color: styles.dateColor }}>
              {event.date}
            </span>
          </>
        )}
...
const defaultStyles: TimelineStyles = {
  lineColor: "#d1d5db",
  activeLineColor: "#E26A2C",
  dotColor: "#d1d5db",
  activeDotColor: "#E26A2C",
  dotSize: "1.5rem",
  titleColor: "inherit",
